...
//IOClose function for unexporting all pins
void IOClose(void)
{
	int FdUnExport;
...
int main(){
	...
	float array[4] = {-9,-9,-9,-9}; 				//For checking floating input values
	...
	if(arr[0] != 0 ){
		...
		if( (arr[0] <= 100 && arr[0] > 0) && arr[1] != -9 && arr[2] != -9 && arr[3] != -9 && (arr[1] < 14 && arr[1] >= 0 ) && (arr[2] < 14 && arr[2] >= 0 ) && (arr[3] < 14 && arr[3] >= 0 ) && (arr[1] != arr[2]) && (arr[2] != arr[3]) && (arr[1] != arr[3] ) ){
			...
			//Thread for detecting mouse click event
			...
			//Enabling respective pins for LEDS
			IOSetup();
			...
			while(1){
				...
				//STEP1
				// loop for toggling the LEDR every cycle
				...
				//STEP2	
				// loop for toggling the LEDG every cycle
				for (j=0;j<k;j++) {
					write(LedG,"1",1);
					usleep(pwm_on);
					write(LedG, "0", 1);
				...
				//STEP4		
				// loop for toggling the LEDR And LEDG every cycle
				for (j=0;j<k;j++) {
					write(LedR,"1",1);
					write(LedG,"1",1);
					usleep(pwm_on);
				...
				//STEP5
				// loop for toggling the LEDR and LEDB every cycle
				for (j=0;j<k;j++) {
					write(LedR,"1",1);
					write(LedB,"1",1);
					usleep(pwm_on);
